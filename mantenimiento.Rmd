...
# Tiempos de actividad de las máquinas
...
# como hacemos la estimación con una única cadena, no podemos calcular el error
cat("\n Tiempo que han funcionado simultáneamente todas las máquinas: ",m)
```
Como hemos obtenido la estimación con una única cadena, no podemos evaluar el error de estimación. Para ello habríamos de simular varias cadenas y aplicar esos cálculos a todas las réplicas. 
...
res = sim.res %>%
  ...
  filter(system0==1) %>%
  summarise(tiempos=sum(dif)*100/t) %>%
  summarise(m=mean(tiempos),error=sd(tiempos))

# como hacemos la estimación con una única caden, no podemos calcular el error
cat("\n Tiempo que han funcionado simultáneamente todas las máquinas: ",res$m,"% (error=",res$error,"%).")
```
...
res=sim.res %>%
  group_by(replication) %>%
  mutate(dif=diff(c(time,t)),
         system234=(system>=2)*1) %>%
  filter(system234==1) %>%
  summarise(tiempos=sum(dif)*100/t) %>%
  summarise(m=mean(tiempos),error=sd(tiempos))

# como hacemos la estimación con una única caden, no podemos calcular el error
cat("\n Tiempo que han estado ocupados simultáneamente los dos reparadores: ",res$m,"% (error=",res$error,"%).")
```
